...
The frontend runs at <http://localhost:3000> and proxies `/api/diagnostics/*`, `/api/interventions/*`, and `/api/designs/*` to the local FastAPI service at `http://127.0.0.1:8000` (override with `COACHLENS_API_ORIGIN`). For a safe end-to-end demo using **synthetic** QA records and **fixed non-AI fixtures**, run `PYTHONPATH=services/api services/api/.venv/bin/python scripts/run_m4_demo.py` from the repository root, then `npm --prefix apps/web run dev`. Approve a diagnosis, then **Propose intervention**, **Validate solution**, and finally **Design Intervention**. Resolution clarity demonstrates practice that reaches training design; a missing workflow prompt demonstrates process correction without training; all-pass follow-up demonstrates investigation without training. Demo outputs pass real validation. See [the M5 guide](docs/m5-training-orchestration.md) and [the M4 guide](docs/m4-review-workspace.md).
