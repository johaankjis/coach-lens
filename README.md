...
CoachLens AI is an evidence-driven performance diagnosis and training platform for the ResultsCX x AWS AI competition. Its planned workflow connects QA evidence to quantitative analysis, human-validated diagnosis, selected interventions, practice, and outcome measurement. Milestone 5 extends the review workspace from intervention review through a proposed intervention and, when appropriate, a training outline, activities, and practice specification. Training stops at Ready for Alignment Review. AWS-1 adds a synthetic-only Amazon Bedrock diagnostic adapter; see [the AWS-1 guide](docs/aws1-bedrock-diagnostics.md) for its privacy boundary and smoke procedure. AWS-3 adds [independent semantic evidence review](docs/aws3-semantic-evidence-validator.md) before the human decision. AWS-4 adds an [Intervention Reasoner and Solution Validator](docs/aws4-intervention-solution-validator.md) after it, so a human-validated diagnosis never becomes training by default. AWS-5 adds a [Bedrock-backed ResultsCX training designer](docs/aws5-training-designer.md) that turns a solution-validated training or practice intervention, and only that, into an outline, activities, knowledge check, scripted practice, and rubric. AWS-4 decides whether design may proceed; AWS-5 decides how. AWS-6 adds an [independent training alignment validator](docs/aws6-alignment-validator.md) that checks whether the generated target behavior, objectives, activities, knowledge check, practice, and rubric actually address the confirmed gap and validated intervention; only an aligned verdict becomes `design_aligned`.
...
The frontend opens on a Home command center (`/`) that summarizes observed QA criteria, the first signal in the backend's failure-count order, its AWS-3 evidence review, human-validation status, AWS-4 intervention and solution-validation state, AWS-5 training package state, and the AWS-6 alignment review state (pending, design aligned, or design questioned). The pipeline strip reads Observed → Diagnosed → Evidence reviewed → Human validated → Intervention proposed → Solution validated → Training generated → Alignment from real backend state; a process correction shows training as not applicable, and a questioned solution shows training as withheld. One context-sensitive next step routes to Agent Insights (`/agent-insights?signal=<id>`), including for Check Alignment and for alignment concerns, or, once a package is design aligned, to Training. The supervisor records the M4 human-validation decision (Approve, Revise, Reject) directly on Home through the same `/hypotheses/{id}` endpoints Agent Insights uses; Home then reloads every stage from the backend. Signal ordering does not establish priority or severity; Priority issues, Agents monitored, and Overall QA remain pending. **Training** and **Role-Play** display the generated AWS-5 package and its facilitator practice script for one signal; **KPI Tracker** states that outcome measurement is pending over the M2 baseline; Reports and Resources are planned pages. Home computes no QA statistics; see [the command center guide](docs/ui1-command-center.md) for the read-model and the AWS-6 stale-review rules.

## M5 design flow and M4 review workspace

The frontend runs at <http://localhost:3000> and proxies `/api/diagnostics/*`, `/api/interventions/*`, and `/api/designs/*` to the local FastAPI service at `http://127.0.0.1:8000` (override with `COACHLENS_API_ORIGIN`). For a safe end-to-end demo using **synthetic** QA records and **fixed non-AI fixtures**, run `PYTHONPATH=services/api services/api/.venv/bin/python scripts/run_m4_demo.py` from the repository root, then `npm --prefix apps/web run dev`. Approve a diagnosis, then **Propose intervention**, **Validate solution**, and finally **Design Intervention**. Resolution clarity demonstrates a skill problem → practice simulation → solution aligned → hands-on training package; a missing workflow prompt demonstrates a process problem → process correction → solution aligned → no training package; all-pass follow-up demonstrates investigation without training. A questioned solution withholds design. Demo outputs pass real validation. The synthetic demo always designs with the controlled M5 fixture; provider-backed AWS-5 generation is exercised by the smoke scripts below and by the normal API and real ResultsCX modes with `COACHLENS_BEDROCK_ENABLED=true`, where it runs only behind the AWS-4 handoff. See [the M5 guide](docs/m5-training-orchestration.md), [the AWS-5 guide](docs/aws5-training-designer.md), and [the M4 guide](docs/m4-review-workspace.md).
