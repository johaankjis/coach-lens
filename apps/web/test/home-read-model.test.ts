...
describe("Home read-model builder", () => {
  ...
  it("shows a generated AWS-5 package with alignment pending and outcome pending (K, N)", () => {
    ...
    expect(selected.nextStep).toMatchObject({ label: "Review training package", href: "/training?signal=sig_top", detail: expect.stringContaining("not deployed") });
    ...
    expect(stage(model, "alignment")).toMatchObject({ status: "pending" });
