import { ALIGNMENT_DIMENSIONS, type AlignmentOutcome, type AlignmentReview, type DesignResult } from "../lib/designs";
...
/**
 * An AWS-6 review of `providerPackage()`: same run, diagnosis, AWS-4 proposal, and solution
 * validation. Any outcome other than "aligned" is `design_questioned`, as the backend guard
 * requires, and names at least one concern so the questioned shape is realistic.
 */
export function alignmentReviewFixture(outcome: AlignmentOutcome = "aligned", overrides: Partial<AlignmentReview> = {}): AlignmentReview {
  const run = designFixture("training").run_id;
  const aligned = outcome === "aligned";
  const dimensions = Object.fromEntries(
    ALIGNMENT_DIMENSIONS.map((name) => [name, { outcome: "aligned", assessment: `${name} holds.`, misaligned_element_ids: [] }]),
  ) as unknown as AlignmentReview["dimensions"];
  if (!aligned)
    dimensions.objective_to_activity = { outcome, assessment: "The activity does not exercise the objective.", misaligned_element_ids: [`${run}/A1`] };
  return {
    alignment_review_id: `alr_${outcome}`, run_id: run, diagnosis_id: "hyp_1", intervention_id: "int_1", solution_validation_id: "sol_1",
    design_digest: "digest-1", assessed: "training_design_package", structural_trace: "structural_references_only",
    overall_outcome: outcome, design_status: aligned ? "design_aligned" : "design_questioned",
    overall_assessment: aligned
      ? "The package trains the confirmed closing behavior."
      : outcome === "insufficient_information"
        ? "The package cannot be judged without the resolution options agents may offer."
        : "The activity trains documentation, not the resolution explanation.",
    dimensions,
    misaligned_element_ids: aligned ? [] : [`${run}/A1`],
    unsupported_assumptions: aligned ? [] : ["Assumes a follow-up message is sent automatically."],
    missing_information: aligned || outcome === "misaligned" ? [] : ["Which resolution options agents may offer."],
    provider_reported_confidence: 0.8,
    provider_metadata: { provider: "Amazon Bedrock", model: "global.anthropic.claude-sonnet-4-6", generation_mode: "provider" },
...
export function insight(overrides: Partial<InsightSources> = {}): InsightSources {
  return { signal: topSignal, record: awaiting, validation: null, intervention: null, design: null, alignment: null, ...overrides };
}
