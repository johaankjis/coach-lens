 ...
 *
 * The AWS-6 alignment review is mapped from the stored review record when the backend returns
 * one for the current design run. Outcome measurement has no backend and renders as pending.
 * Nothing in this file invents a value for either.
 */
import type { AlignmentOutcome, AlignmentReview, DesignResult } from "../designs";
import {
  label,
  validated,
...
/* ---------- AWS-6 alignment review and outcome ----------------------------------------- */

/** The stored AWS-6 review of the current package, summarized. Never a human decision. */
export type AlignmentReviewSummary = {
  reviewId: string;
  runId: string;
  /** Backend `overall_outcome`; only "aligned" is design_aligned. */
  ...
  origin: DiagnosisOrigin;
  provider: string;
  /** Element ids named by the reviewer, shortened to the run-local part for display. */
...
/**
 * `blocked`: no generated package. `not_applicable`: the validated intervention is not training.
 * `pending`: a package exists and no current AWS-6 review does. `design_aligned` /
 * `design_questioned` carry the backend `design_status` of the review for this exact run;
 * partially aligned, misaligned, and insufficient information are all `design_questioned`.
 * A review the backend reports as stale, or one for another run, never reaches this stage.
 */
export type AlignmentStage =
  | { state: "blocked"; reason: string }
  | { state: "not_applicable"; reason: string }
  | { state: "pending"; reason: string }
  | ({ state: "design_aligned" } & AlignmentReviewSummary)
  | ({ state: "design_questioned" } & AlignmentReviewSummary);
...
export type InsightSources = {
  ...
  /** AWS-6 stored review for the design run, when the backend returned a current one. */
...
/**
 * The AWS-6 review only counts when it belongs to the package Home summarizes: same validated
 * diagnosis, same design run, and the same AWS-4 proposal and solution validation the package
 * was built from. The backend already refuses a review whose `design_digest` no longer matches
 * the stored run (409 `alignment_review_stale`), so a review that arrives here passed that check.
 */
function alignmentFor(
  record: RecordState | null,
  intervention: InterventionRecord | null,
  design: DesignResult | null,
  review: AlignmentReview | null,
): AlignmentReview | null {
  if (!record || !intervention || !design?.training_design || !review) return null;
  if (review.diagnosis_id !== record.provider_hypothesis.hypothesis_id || review.diagnosis_id !== design.diagnosis_id) return null;
  if (review.run_id !== design.run_id) return null;
  if (review.intervention_id !== intervention.proposal.intervention_id ||
      review.solution_validation_id !== intervention.solution_validation?.solution_validation_id) return null;
  return review;
}

function reviewSummary(review: AlignmentReview): AlignmentReviewSummary {
  const origin = stageOrigin(review.provider_metadata);
  const shortId = (id: string) => (id.startsWith(`${review.run_id}/`) ? id.slice(review.run_id.length + 1) : id);
  return {
    reviewId: review.alignment_review_id,
    runId: review.run_id,
    outcome: review.overall_outcome,
    outcomeLabel: label(review.overall_outcome),
    assessment: review.overall_assessment,
    confidence: {
      value: String(review.provider_reported_confidence),
      wording:
        origin === "fixture"
          ? "Fixed synthetic fixture value. No model reported it, and it is not a calibrated probability."
          : "Validator self-report for this review. It is not a statistically calibrated probability.",
    },
    origin,
    provider: review.provider_metadata.provider,
    problematicElements: review.misaligned_element_ids.map(shortId),
    unsupportedAssumptions: review.unsupported_assumptions,
    missingInformation: review.missing_information,
    reviewedAt: review.created_at,
  };
}

function alignmentStage(training: TrainingStage, review: AlignmentReview | null): AlignmentStage {
  if (training.state === "generated") {
    if (review) return { state: review.design_status, ...reviewSummary(review) };
    return { state: "pending", reason: "Not yet reviewed. The independent AWS-6 alignment review of this package against the validated diagnosis has not been run." };
  }
...
function pipeline(
  ...
  const aligned: PipelineStep = {
    ...
    ...({
      ...
      design_aligned: { tone: "validated", text: "Design aligned" },
      design_questioned: { tone: "caution", text: "Design questioned" },
...
function nextStep(
  ...
  if (training.state === "generated") {
    // Check Alignment lives in Agent Insights; Training displays the package and any review.
    if (alignment.state === "design_aligned")
      return {
        label: "Review aligned training package",
        detail: "The AWS-6 review found the package addresses the confirmed gap. It is generated, not deployed; no outcome has been measured.",
        href: training.trainingHref,
      };
    if (alignment.state === "design_questioned")
      return {
        label: "Review alignment concerns",
        detail: `The AWS-6 review found the package ${alignment.outcomeLabel.toLowerCase()}. Review the concerns in Agent Insights; nothing is deployed.`,
        href,
      };
    return {
      label: "Review training alignment",
      detail: "An AWS-5 package is generated, not deployed. Run Check Alignment in Agent Insights for the independent AWS-6 review.",
      href,
    };
  }
...
export function buildPriorityInsight(source: InsightSources): PriorityInsight {
  ...
  const alignment = alignmentStage(training, alignmentFor(record, interventionRecord, design, source.alignment));
  return {
    ...
    nextStep: nextStep(signal, record, validation, intervention, training, alignment),
...
export function buildDownstream(insight: PriorityInsight | null): DownstreamStage[] {
  ...
  const alignment: DownstreamStage = {
    ...
    ...(!insight
      ? { status: "blocked" as const, detail: "Requires a generated training package." }
      : insight.alignment.state === "blocked"
        ? { status: "blocked" as const, detail: insight.alignment.reason }
      : insight.alignment.state === "not_applicable"
        ...
        : insight.alignment.state === "pending"
          ? { status: "pending" as const, detail: insight.alignment.reason, href: insight.reviewHref, hrefLabel: "Review training alignment" }
          : insight.alignment.state === "design_aligned"
            ? {
                status: "design_aligned" as const,
                detail: `${insight.alignment.outcomeLabel}. ${insight.alignment.assessment} Semantic review, not a human decision; nothing is deployed and no outcome is measured.`,
                href: insight.training.state === "generated" ? insight.training.trainingHref : insight.reviewHref,
                hrefLabel: "Review aligned training package",
              }
            : {
                status: "design_questioned" as const,
                detail: `${insight.alignment.outcomeLabel}. ${insight.alignment.assessment}`,
                href: insight.reviewHref,
                hrefLabel: "Review alignment concerns",
              }),
  };
  return [
    intervention,
