...
function downstreamPill(stage: DownstreamStage): { tone: StatusTone; text: string } {
  switch (stage.status) {
    ...
    case "pending":
      return { tone: "pending", text: "Pending · not yet reviewed" };
    case "design_aligned":
      return { tone: "validated", text: "Design aligned · not deployed" };
    case "design_questioned":
      return { tone: "caution", text: "Design questioned" };
  }
}
    ...
    <>
      ...
      <div className="insight-field">
        ...
        {alignment.state === "design_aligned" || alignment.state === "design_questioned" ? (
          <>
            <strong>{alignment.assessment}</strong>
            <small>{alignmentFlagSummary(alignment)}</small>
            <small>
            ...
            <small>
              {alignment.origin === "fixture" ? "Fixed fixture review" : "Independent AI semantic review"} of the
              package against the confirmed gap. Not a human decision, not deployment, and not an outcome.
            </small>
          </>
        ) : (
