...
import { alignmentFlagSummary, alignmentPill, interventionPill, Pill, Pipeline, trainingPill } from "../command-center";
...
export function InterventionContext({ insight }: { insight: PriorityInsight }) {
  ...
  const alignmentTone = alignmentPill(insight.alignment);
  const proposed =
    ...
    <section className="home-card stage-context" aria-labelledby="context-title">
      ...
      <div className="insight-grid">
        ...
        <div className="insight-field">
          ...
          {insight.alignment.state === "design_aligned" || insight.alignment.state === "design_questioned" ? (
            <small>
              {alignmentFlagSummary(insight.alignment)}. AWS-6 semantic review; not a human decision, not deployment,
              and not an outcome.
            </small>
          ) : (
