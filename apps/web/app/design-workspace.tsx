...
export default function DesignWorkspace({ result }: { result: DesignResult }) {
  ...
  return (
    <section className="design-workspace" aria-label="Proposed intervention design">
      <div className="section-kicker">{result.generation_mode === "controlled_fixture" ? "CONTROLLED NON-AI DEMO PROPOSAL" : decision.decision_type === "training" ? "AI-GENERATED DESIGN PROPOSAL" : "AI-GENERATED INTERVENTION PROPOSAL"}</div>
      <h2>Intervention decision: {label(decision.decision_type)}</h2>
      <p>{decision.rationale}</p>
      <p className="quiet">Validated diagnosis {shortId(result.diagnosis_id)} · {result.approved_diagnosis.human_revised ? "Human revision used" : "Approved original used"} · Evidence: {decision.evidence_refs.map((ref) => shortId(ref.item_id)).join(", ")}</p>
      {decision.risks.length > 0 && <><h3>Risks / limitations</h3><ul>{decision.risks.map((risk, i) => <li key={i}>{risk}</li>)}</ul></>}
      {decision.decision_type === "investigate" && <><h3>Additional evidence required</h3><p>Current evidence is insufficient to select training.</p><ul>{decision.unresolved_questions.map((question, i) => <li key={i}>{question}</li>)}</ul></>}
      {decision.decision_type === "non_training" && <h3>Training not selected</h3>}
      <h3>{design ? "Proposed next step" : "Recommended next actions"}</h3>
      ...
      {design && <>
        ...
        <p className="quiet">Proposed training design; not validated or alignment-reviewed. No learner has been scored.</p>
