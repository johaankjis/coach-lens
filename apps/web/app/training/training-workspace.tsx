...
import { AlignmentSummary } from "../alignment-check";
  ...
  return (
    ...
    >
      {(model) => {
        ...
        const { training, alignment } = insight;
        // The read-model already matched the stored review to this design run and its validated
        // intervention; render the raw record only when it is the one the model accepted.
        const stored = load.sources?.priority?.alignment ?? null;
        const currentReview =
          stored && (alignment.state === "design_aligned" || alignment.state === "design_questioned") &&
          stored.alignment_review_id === alignment.reviewId && stored.run_id === design?.run_id
            ? stored
            : null;
        return (
          <>
            ...
            {training.state === "generated" && design?.training_design ? (
              <>
                <section className="home-card training-package" aria-labelledby="package-title">
                  ...
                  <p className="quiet">
                    Design status: {training.designStatus.replaceAll("_", " ")}.{" "}
                    {currentReview
                      ? currentReview.design_status === "design_aligned"
                        ? "The independent AWS-6 alignment review found the design aligned; it is still not deployed and no outcome is measured."
                        : "The independent AWS-6 alignment review questioned the design."
                      : "Independent AWS-6 alignment review is pending; run Check Alignment in Agent Insights."}{" "}
                    Hands-on practice is shown on the Role-Play page.
                  </p>
                  {currentReview && <AlignmentSummary review={currentReview} detailHref={insight.reviewHref} />}
                  <DesignWorkspace result={design} signalLabel={insight.criterion} alignmentReview={currentReview} hidePractice hideAlignmentDetail />
                </section>
              </>
