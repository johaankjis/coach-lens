...
counts. It receives the confirmed gap and the AWS-4 solution-validated intervention as text,
the versioned ResultsCX design guidance as instructions, and returns one strict JSON package
with short local identifiers that this module prefixes with the run ID before the M5
structural validator runs. There is no repair, retry, or fixture fallback: anything the
contract refuses is discarded and nothing is stored. Whether design may run at all was
decided upstream by AWS-4; this module only decides how.
"""
...
The validated intervention states its type (training or practice_simulation), the
recommendation an intervention reasoner made, the target behavior change it named, and its
rationale. A separate solution review already found it aligned with the confirmed cause.
Design from the confirmed gap and the validated intervention only. Build the target behaviors
around the stated target change. Do not rediagnose, do not propose a different cause, do not
choose a different intervention, and do not widen the training beyond that gap. Every target
...
INTERVENTION_KEYS = frozenset({"reference", "intervention_type", "training_focus", "recommendation",
                               "target_change", "rationale"})
...
def build_designer_request(intervention: ValidatedTrainingIntervention, qa_criterion: str) -> dict:
    ...
    request = {
        ...
        "validated_intervention": {
            ...
            "target_change": intervention.target_change,
    ...
    return {
        ...
        "design_basis": {
            ...
            "intervention": {"run_id": run, "decision_type": DecisionType.TRAINING.value,
                             ...
                             "solution_validation_id": intervention.solution_validation_id,
                             "solution_alignment": intervention.solution_alignment,
                             "training_design_gate": intervention.training_design_gate,
                             "training_focus": intervention.training_focus.value,
                             "validation_source": intervention.validation_source,
                             "summary": intervention.recommendation,
                             "target_change": intervention.target_change},
...
class BedrockTrainingDesigner:
    ...
    def prepare(self, context: DesignInput, decision: InterventionDecision
        ...
        """Consume the AWS-4 handoff, gate, screen, and project. Raises before any client exists."""
        ...
        if _sensitive_diagnosis_text(self._diagnostics, [request["confirmed_performance_gap"]["qa_criterion"],
                                                         ...
                                                         request["validated_intervention"]["recommendation"],
                                                         request["validated_intervention"]["target_change"],
                                                         request["validated_intervention"]["rationale"],
