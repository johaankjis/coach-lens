...
The separate system prompt contains the review principles and a response contract rendered from `ValidatorResponse`. Converse uses one user text block, temperature `0`, and `maxTokens` `1400`. Only `stopReason: end_turn` is accepted.
