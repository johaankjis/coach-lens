...
AWS-4 supplies the intervention step through `ValidatedInterventionHandoff` and decides whether training design may proceed; AWS-5 adds a provider-backed training designer, optional design-basis and scripted-practice fields, and a service-built alignment trace, and decides how a permitted training or practice intervention becomes a package. `DesignService` runs a provider-backed designer only behind the AWS-4 handoff and a `permitted` decision (`training_design_not_permitted`, 409, otherwise); the controlled fixture path is exempt because it performs no inference. See [the AWS-4 guide](aws4-intervention-solution-validator.md) and [the AWS-5 guide](aws5-training-designer.md). Everything below still holds.
...
`InterventionDecision` is an immutable, typed proposal: `training`, `non_training`, or `investigate`, with rationale, approved evidence citations, risks, unresolved questions, proposed next actions, and the provider's self-reported `provider_metadata` (provider and model) for audit. Since AWS-4 it also carries optional `intervention_type`, `recommendation`, `target_change`, `solution_alignment`, `intervention_id`, `solution_validation_id`, and `training_design_gate`, which must agree with `decision_type` and with each other when present. In the normal API and both demos the intervention step is the AWS-4 handoff (`ValidatedInterventionHandoff`) rather than a fresh provider decision: it projects the stored, solution-reviewed intervention onto this contract and refuses to run design until an intervention is proposed and the solution review finds it `aligned`. All `solution_questioned` outcomes stop before M5, including non-training proposals; see [AWS-4](aws4-intervention-solution-validator.md). It is AI- or fixture-proposed and is **not** human validated; the only human decision in the chain is the M3 diagnosis approval. No cause-domain → intervention rule exists. The decision provider must explain its choice from the approved diagnosis. For `non_training`, the result contains recommended actions and no training artifact. For `investigate`, it must contain unresolved questions and likewise has no training artifact. Only `training` calls the training designer. These alternatives prevent a validated diagnosis from automatically becoming unnecessary training.
