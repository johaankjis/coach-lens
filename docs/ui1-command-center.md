...
5. **Downstream stages**: Intervention (AWS-4), Training (AWS-5), Alignment (AWS-6), Outcome. Intervention renders blocked, not started, proposed, solution validated, or solution questioned from the AWS-4 record. Training renders blocked, awaiting solution, withheld (solution questioned a training proposal), not applicable (non-training or investigate decision), permitted but not generated, or generated from the AWS-4 gate and the AWS-5 design result. Alignment renders blocked, not applicable, pending / not yet reviewed, **design aligned**, or **design questioned** from the stored AWS-6 review (`GET /designs/diagnoses/{id}/alignment-review`): an `aligned` overall outcome is design aligned; partially aligned, misaligned, and insufficient information are all design questioned. The card carries the reviewer's assessment, its non-calibrated confidence, and the counts of problematic elements, unsupported assumptions, and missing information. Outcome is always measurement pending; a design being aligned says nothing about effectiveness.
...
7. **Recommended next step**: one context-sensitive action derived from the record, intervention, and training states: review observed evidence and run diagnosis; review diagnosis; approve human revision; request another hypothesis; generate intervention; validate solution; review solution concerns; generate training; review validated process intervention; review investigation recommendation; review training alignment (a package exists and no AWS-6 review does; routes to Agent Insights, where Check Alignment lives); review aligned training package (routes to Training); review alignment concerns (design questioned; routes to Agent Insights). Every other action routes to Agent Insights, where the real buttons live. No action implies deployment.
...
- Human approval of the diagnosis is not an intervention decision and not solution validation. Solution validation is an AI (or fixture) review, not a human approval and not training alignment. Generated training is not deployed training; no learner has taken it. Design aligned is the AWS-6 semantic verdict on the package, not a human decision, not deployment, and not an outcome. No post-training improvement is shown anywhere. The phrases evidence validated, human validated, solution validated, training generated, design aligned, and outcome validated are kept distinct.
- The AWS-6 review counts only when it belongs to the selected signal's validated diagnosis, the current design run, and the AWS-4 proposal and solution validation the package was built from (`alignmentFor` in `read-model.ts`). The backend enforces the package digest: `GET .../alignment-review` answers 409 `alignment_review_stale` when the stored review no longer matches the run, and the loader treats that, like 404, as "no current review", so the stage shows pending rather than an outdated verdict.
...
`HomeSources` is the raw backend material: the runtime mode, the signal list, and for the selected signal (`InsightSources`) its latest record, its stored semantic review (404 means none), and, when the record is validated, its AWS-4 intervention record and its AWS-5 / M5 design result (404 means the stage has not run), and, when that result holds a training package, its AWS-6 alignment review (404 means none; 409 stale means none that is current). `loadHomeSources(signalId)` selects a requested signal for the Training and Role-Play pages and reports an unknown id through `requested.found` while falling back to the first signal. `HomeReadModel` is display-ready and fully typed: `Provenance`, `SummaryMetric[]` with a `MetricReading` union (`available | pending | unavailable`), `PerformanceGap[]`, `PriorityInsight | null`, and `DownstreamStage[]`.
...
| Alignment stage | wired to `GET /designs/diagnoses/{id}/alignment-review` (fetched only when a training package exists) | — |
