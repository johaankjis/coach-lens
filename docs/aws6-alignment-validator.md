...
A design without provider-generated AWS-5 training, its stored design basis, or the AWS-4 handoff fields has no integrated package and validated intervention to align against and is refused with `alignment_review_not_permitted` (409) before any call. A non-training result is refused with `no_training_design` (409).
