...
5. The UI uses the returned `DiagnosticRecord` from each mutation. A failed operation never fabricates success. A 409 conflict reloads the record. Rejected diagnoses are never Ready for Design. Approval displays available reviewer and timestamp metadata.
